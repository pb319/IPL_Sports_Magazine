-- Creating Database for Importing CSV files
CREATE DATABASE IF NOT EXISTS ipl;

-- Altering Complex names ito simple one
...
-- Data Exploration
	...
	-- DIM_MATCH
		...
		COUNT(DISTINCT(team1)) as No_of_Teams, -- `No_of_Teams` stands for the total Number of Teams participated in these three years
		...
		DESC dim_match; -- Gives description of each column
		
		-- FROM here we come to know that 10 teams participated in 206 matches so far in these three years

		SELECT *
		FROM dim_match
		WHERE match_id = 'T208201'; -- this query has been used to understand a specific field with different entry that has been changed as follows 
		
		set sql_safe_updates =0; -- safe mode revoked
		-- this field may arise some problem while splitting and changing data type hence changed to 'May 29, 2023' from `'May 28-29, 2023'`
	...
	-- dim_players
		...
		FROM dim_players; 
        -- Datavalidation has been checked i.e. Total Teams = 10
